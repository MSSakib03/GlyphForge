...
export default function GlyphForge() {
  ...
  // React 18 Transition for smoother search/filtering
  ...
  // --- OPTIMIZATION: Filtered Glyphs with Search ---
  ...
  // --- OPTIMIZATION: Handle Search Input via Transition ---
  ...
  const handleModeSwitch = (mode) => {
     ...
     setTimeout(() => { setFilterMode(mode); setIsModeLoading(false); }, 50);
  };
  ...
  // --- SMART & FAST CLIP CHECK (Math + Path Fallback) ---
  const checkClip = useCallback((g, explicitOverrides = null) => {
      ...
      // --- PHASE 1: MATH-BASED CHECK (Fastest) ---
      const raw = g.glyph.getBoundingBox();
      const rawW = raw.x2 - raw.x1;
      const rawH = raw.y2 - raw.y1;
      const pixelW = rawW * fontScale;
      const pixelH = rawH * fontScale;
      
      const xPos = (s.canvasWidth / 2) + s.translateX - (pixelW / 2) - (raw.x1 * fontScale);

      let yPos;
      if (s.positioning === 'center') {
        yPos = (s.canvasHeight / 2) + s.translateY + (pixelH / 2) - (raw.y2 * fontScale);
      ...
      // Calculate final bounding box on canvas
      const finalX1 = xPos + (raw.x1 * fontScale);
      const finalY1 = yPos - (raw.y2 * fontScale); 
      const finalX2 = xPos + (raw.x2 * fontScale);
      const finalY2 = yPos - (raw.y1 * fontScale);

      const realY1 = Math.min(finalY1, finalY2);
      const realY2 = Math.max(finalY1, finalY2);

      const TOLERANCE = 0.1;
      const isClippedMath = finalX1 < -TOLERANCE || realY1 < -TOLERANCE || finalX2 > (s.canvasWidth + TOLERANCE) || realY2 > (s.canvasHeight + TOLERANCE);

      if (!isClippedMath) return false;

      // --- PHASE 2: PATH-BASED CHECK (Accurate but Slow) ---
      const measurePath = g.glyph.getPath(xPos, yPos, scaledFontSize);
  ...
  // --- TIME-SLICED FILE PROCESSOR ---
  const processFiles = (fileList) => {
    if (!isDataReady) {
        alert("System is still loading data resources. Please wait a moment.");
        return;
    }
            ...
            try {
                ...
                const processGlyphBatch = () => {
                    ...
                    if (gIndex < totalGlyphs) {
                        ...
                        newFonts.push({ name: file.name.replace(/\.(ttf|otf|woff)$/i, ''), fileName: file.name, fontObj: parsedFont, glyphs: glyphList });
                        fileIndex++;
                        setTimeout(processNextFile, 0); 
                    }
                };
  ...
  const switchFont = (index) => { setActiveFontIndex(index); setSelectedIndices(new Set()); setOverrides({}); setLastSelectedIndex(null); saveToHistory(globalSettings, {}); };
  const removeFont = (e, index) => { e.stopPropagation(); const newList = fontList.filter((_, i) => i !== index); setFontList(newList); if (activeFontIndex >= newList.length) { setActiveFontIndex(Math.max(0, newList.length - 1)); } };
  ...
  const executeDeleteGlyphs = (indicesToDelete) => {
    scanCancelRef.current = true;
  ...
  const requestDeleteGlyphs = (indicesToDelete) => {
    setConfirmState({
        isOpen: true,
        title: "Remove Glyphs",
        message: `Are you sure you want to remove ${indicesToDelete.size} selected glyphs from the list?`,
        isDangerous: true,
        onConfirm: () => {
            executeDeleteGlyphs(indicesToDelete);
            closeConfirm();
        }
    });
  };

  const requestDeleteBlock = (block) => {
    setConfirmState({
        isOpen: true,
        title: "Delete Block",
        message: `Are you sure you want to delete the entire block "${block.name}"? This will remove ${block.count} glyphs.`,
        isDangerous: true,
        onConfirm: () => {
            const indicesToRemove = new Set(block.glyphs.map(g => g.index));
            executeDeleteGlyphs(indicesToRemove);
  ...
  const handleUpdateGlyphPos = useCallback((index, tx, ty) => {
     setOverrides(prev => {
        const newOverrides = { ...prev, [index]: { ...(prev[index] || globalSettings), translateX: tx, translateY: ty } };
  ...
  const toggleSelection = useCallback((index, multiSelect) => {
    setSelectedIndices(prev => {
        ...
        else { if (!multiSelect) newSet.clear(); newSet.add(index); }
        return newSet;
    });
  ...
  const performAutoFit = () => {
    ...
    targets.forEach(g => {
        ...
        newOverrides[g.index] = { ...s, scale: optimalScale, translateX: 0, translateY: 0, positioning: 'center' };
    });
    setOverrides(newOverrides);
  ...
  const generateSVGString = (glyph, settings) => {
    ...
    if (settings.renderMode === 'outline') {
        fill = 'none'; strokeColor = settings.color; strokeW = settings.strokeWidth <= 1 ? 1 : finalStrokeWidth;
    } else if (settings.renderMode === 'negative') {
        fill = settings.backgroundColor === 'transparent' ? '#ffffff' : settings.backgroundColor; strokeColor = fill; strokeW = finalStrokeWidth; bgRect = `<rect width="100%" height="100%" fill="${settings.color}"/>`; 
    } else {
        fill = settings.color; strokeColor = settings.strokeWidth < 1 ? settings.backgroundColor : settings.color; strokeW = finalStrokeWidth;
    }
  ...
  const handleFloatingPointerDown = (e) => {
      ...
      const element = e.currentTarget; element.setPointerCapture(e.pointerId);
      const rect = element.getBoundingClientRect();
      const parentRect = element.offsetParent.getBoundingClientRect();
      setFloatingPos({ x: rect.left - parentRect.left, y: rect.top - parentRect.top });
  ...
  const calculateTotalPages = () => {
    ...
    if (viewMode === 'block' && activeBlock && activeBlock.glyphs.length > LARGE_BLOCK_THRESHOLD) return Math.ceil(activeBlock.glyphs.length / LARGE_BLOCK_PAGE_SIZE);
    if (viewMode === 'errors' && errorIndices.size > ERROR_VIEW_PAGE_SIZE) return Math.ceil(errorIndices.size / ERROR_VIEW_PAGE_SIZE);
    return 0;
  };
  ...
  return (
    <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={cn("h-screen flex bg-gray-50 dark:bg-gray-950 text-gray-800 dark:text-gray-100 font-sans relative", darkMode ? "dark" : "")}>
      ...
      <Sidebar 
        ...
        blocks={blocks} activeBlock={activeBlock} setActiveBlock={setActiveBlock} paginatedBlocks={paginatedBlocks} currentPage={currentPage} setCurrentPage={setCurrentPage} setViewMode={setViewMode} 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative transition-all duration-300">
        <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-6 shrink-0 z-10 gap-4">
          <div className="flex items-center gap-4 flex-1">
             ...
             <div className="relative max-w-md w-full">
                ...
                <input type="text" placeholder="Search glyphs (e.g. 'A', '0041', 'U+0995')" value={searchTerm} onChange={handleSearchChange} className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-gray-800 border-none rounded-lg text-sm focus:ring-2 focus:ring-violet-500 outline-none" />
             </div>
             {viewMode !== 'all' && (<Button variant="secondary" size="sm" onClick={() => { setViewMode('all'); setActiveBlock(null); }}><ChevronLeft size={14} /> Back to All</Button>)}
                 ...
                 <div>
                     <div className="mb-6 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            {viewMode === 'errors' && (<div className="flex items-center gap-2 ml-4 border-l pl-4 border-gray-300 dark:border-gray-700"><Button variant="ghost" size="xs" onClick={() => { const visible = getVisibleGlyphs(); const newSet = new Set(selectedIndices); visible.forEach(g => newSet.add(g.index)); setSelectedIndices(newSet); }} title="Select all on this page"><CheckSquare size={14}/> Select Page</Button><Button variant="ghost" size="xs" onClick={() => { const newSet = new Set(selectedIndices); errorIndices.forEach(idx => newSet.add(idx)); setSelectedIndices(newSet); }} title="Select all errors"><CheckSquare size={14}/> Select All ({errorIndices.size})</Button></div>)}
                        </div>
                        {viewMode === 'errors' && errorIndices.size > ERROR_VIEW_PAGE_SIZE && (<span className="text-sm text-gray-500 font-mono"><span className="ml-2 bg-red-100 text-red-700 px-2 py-0.5 rounded text-xs">Showing {(currentPage - 1) * ERROR_VIEW_PAGE_SIZE + 1} - {Math.min(currentPage * ERROR_VIEW_PAGE_SIZE, errorIndices.size)} of {errorIndices.size} Errors</span></span>)}
                        {viewMode === 'block' && <span className="text-sm text-gray-500 font-mono">{formatRange(activeBlock.originalStart || activeBlock.start, activeBlock.originalEnd || activeBlock.end)} {activeBlock.glyphs.length > LARGE_BLOCK_THRESHOLD && (<span className="ml-2 bg-violet-100 text-violet-700 px-2 py-0.5 rounded text-xs">Showing {(currentPage - 1) * LARGE_BLOCK_PAGE_SIZE + 1} - {Math.min(currentPage * LARGE_BLOCK_PAGE_SIZE, activeBlock.glyphs.length)} of {activeBlock.glyphs.length}</span>)}</span>}
                     </div>
                     <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(120px,1fr))] pb-32">
                        {getVisibleGlyphs().map(g => (<GlyphCard key={g.index} g={g} settings={getSettingsForGlyph(g.index)} isSelected={selectedIndices.has(g.index)} isError={errorIndices.has(g.index)} toggleSelection={toggleSelection} onUpdatePosition={handleUpdateGlyphPos} onDragEnd={handleDragEnd} />))}
                     </div>
                 </div>
               )}
               {viewMode === 'all' && (
                 <div className="pb-32 min-h-full">
                     {paginatedBlocks[currentPage - 1]?.map((block, idx) => (<BlockSection key={`${currentPage}-${idx}`} block={block} collapsedBlocks={collapsedBlocks} setCollapsedBlocks={setCollapsedBlocks} errorIndices={errorIndices} selectedIndices={selectedIndices} setSelectedIndices={setSelectedIndices} deleteBlock={requestDeleteBlock} handleDownload={handleDownload} setViewMode={setViewMode} setActiveBlock={setActiveBlock} getSettingsForGlyph={getSettingsForGlyph} toggleSelection={toggleSelection} handleUpdateGlyphPos={handleUpdateGlyphPos} handleDragEnd={handleDragEnd} setCurrentPage={setCurrentPage} />))}
        ...
        {font && (<PaginationControls currentPage={currentPage} setCurrentPage={setCurrentPage} totalPages={calculateTotalPages()} />)}
